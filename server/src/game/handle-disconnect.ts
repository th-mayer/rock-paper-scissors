...
export function handleDisconnect(socket_id: string, io: Server) {
    ...
    if (match_id) {
        if (running_matches[match_id]){ // If Client disconnected from a running match
            ...
            if (player1socket) {
                ...
                if (socket_in_matches[player1socket.id]){
                    if (socket_id != player1socket.id) dbUsers.updateWinItemCoin(running_matches[match_id].player1.userID, true); // Match counts as a win for remaining Player
            ...
            if (player2socket) {
                ...
                if (socket_in_matches[player1socket.id]) {
                    if (socket_id != player2socket.id) dbUsers.updateWinItemCoin(running_matches[match_id].player2.userID, true);
        ...
        } else if (open_matches[match_id]) { // If Client disconnected from a running match
